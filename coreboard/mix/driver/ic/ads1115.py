...
class PRMADS1115(object):
    '''
    ADS1115 function class

    :param     dev_addr: hexmial,             ads1115 i2c bus device address
    :param     i2c_bus:  Instance/None,       i2c bus class instance, if not using, will create emulator
    :param     fs:       string('fs0'-'fs7'), full scale input config
    ...
    # 'fs1' means 4.096V external
    ads1115 = ADS1115(address, i2c_bus, 'fs1')
    # read voltage of ads1115 '0_1'
    volt = ads1115.read_volt(0)
    print("the voltage is {}".format(volt))

    '''
    rpc_public_api = [ "write_register", "read_register", "set_sampling_rate", "get_sampling_rate", "read_volt"
                       ]
    ...
    def write_register(self, addr, value):
        ...
        :param     addr:   hexmial(0-0xff),    register address
        :param     value:  hexmial(0-0xffff),  value to be write to register
        :example:
    ...
    def read_volt(self, channel):
        ...
        :param     channel:    int (0/1..),    0 mean '0_1',
                                                1 mean '0_3',
                                                2 mean '1_3',
                                                3 mean '2_3',
                                                4 mean '0_G',
        ...
        lasttime = pyb.millis()
        while pyb.elapsed_millis(lasttime) < ADS1115Def.DEFAULT_TIMEOUT:
            rd_data = self.read_register(ADS1115Def.CONFIG_REGISTER)
            if rd_data & ADS1115Def.OS_MASK == ADS1115Def.OS_CONV_COMPLETED:
                break
            pyb.delay(ADS1115Def.DEFAULT_DELAY)

        if pyb.elapsed_millis(lasttime) - lasttime > ADS1115Def.DEFAULT_TIMEOUT:
            raise ADS1115Exception(
                self._dev_addr, 'Wait ADS1115 conversion completed timeout.')
